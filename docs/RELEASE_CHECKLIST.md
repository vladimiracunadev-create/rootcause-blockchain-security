...
impide publicar un `v0.3.0` que por dentro dice `0.1.0`.
...
| 2 | Los tres artefactos están publicados | `gh release view v0.3.0 --json assets` |
| 3 | Los checksums son legibles | `gh release download v0.3.0 --pattern SHA256SUMS.txt` |
...
- **El tag ya está creado y el workflow falló.** Corrige, borra el tag remoto
  (`git push --delete origin v0.3.0`), borra el local y vuelve a empezar. No
